...
export const MakePicks = () => {
  ...
  useEffect(() => {
    ...
    const getAvailableGames = async () => {
      const response = await fetch(
        `https://api.chadssupercontest.net:8080/getLines/${currentWeekNumber}`
      );
    ...
    if (authCtx.isLoggedIn && authCtx.username) {
      const getPrevSubmittedPicks = async () => {
        const response = await fetch(
          `https://api.chadssupercontest.net:8080/getPicks/${authCtx.username}?weekNumber=${currentWeekNumber}`,
          {
            headers: {
  ...
  const submitPicksHandler = () => {
    ...
    fetch('https://api.chadssupercontest.net:8080/submitPicks', {
      method: 'POST',
      body: JSON.stringify({
