...
export const MakePicks = () => {
  ...
  useEffect(() => {
    const getCurrentWeekNumber = async () => {
      const response = await fetch(
        'https://api.chadssupercontest.net/getCurrentWeekNumber'
      );
  ...
  useEffect(() => {
    ...
    const getAvailableGames = async () => {
      const response = await fetch(
        `https://api.chadssupercontest.net/getLines/${currentWeekNumber}`
      );
    ...
    if (authCtx.isLoggedIn && authCtx.username) {
      const getPrevSubmittedPicks = async () => {
        const response = await fetch(
          `https://api.chadssupercontest.net/getPicks/${authCtx.username}?weekNumber=${currentWeekNumber}`,
          {
            headers: {
  ...
  const submitPicksHandler = () => {
    ...
    fetch('https://api.chadssupercontest.net/submitPicks', {
      method: 'POST',
      body: JSON.stringify({
